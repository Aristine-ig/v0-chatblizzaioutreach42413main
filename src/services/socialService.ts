...
export const socialService = {
  ...
  async getFollowers(userId: string) {
    const { data, error } = await supabase.from("user_follow").select("follower_id").eq("following_id", userId)
    if (error) throw error
    return data?.map((f) => f.follower_id) || []
  },

  async getFollowing(userId: string) {
    const { data, error } = await supabase.from("user_follow").select("following_id").eq("follower_id", userId)
    if (error) throw error
  ...
  async deleteSharedMeal(sharedMealId: string) {
    const { error } = await supabase.from("shared_meals").delete().eq("id", sharedMealId)
    if (error) throw error
  },
  ...
  async getReactions(sharedMealId: string) {
    const { data, error } = await supabase.from("meal_reactions").select("*").eq("shared_meal_id", sharedMealId)
    if (error) throw error
  ...
  async getUserStats(userId: string): Promise<UserStats> {
    const [{ data: followers }, { data: following }, { data: sharedMeals }, { data: foodLogs }] = await Promise.all([
