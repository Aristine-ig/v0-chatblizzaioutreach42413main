...
import { socialService, type SharedMeal, type UserStats } from "../services/socialService"
...
export default function SocialView({ userId, inline = false }: SocialViewProps) {
  ...
  const renderMealCard = (meal: SharedMeal, showDelete = false) => (
    <div key={meal.id} className="bg-white rounded-xl shadow-md p-4 border border-gray-200">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h3 className="font-semibold text-gray-800">{meal.food_logs?.food_name || "Unknown Food"}</h3>
          <p className="text-sm text-gray-600">{meal.food_logs?.calories || 0} calories</p>
        </div>
        {showDelete && (
          <button
      ...
      {meal.caption && <p className="text-gray-700 text-sm mb-3">{meal.caption}</p>}

      {meal.food_logs?.image_url && (
        <img
          src={meal.food_logs.image_url || "/placeholder.svg"}
          alt="Meal"
        ...
        <div className="flex gap-2 bg-white p-1 rounded-lg shadow-sm">
          <button
            ...
            className={`flex-1 py-2 rounded-lg transition font-medium ${
              tab === "feed" ? "bg-emerald-600 text-white" : "text-gray-600 hover:text-gray-800"
            }`}
          >
            Feed
          </button>
          <button
            ...
            className={`flex-1 py-2 rounded-lg transition font-medium ${
              tab === "shared" ? "bg-emerald-600 text-white" : "text-gray-600 hover:text-gray-800"
            }`}
          >
            My Meals
          </button>
          <button
            ...
            className={`flex-1 py-2 rounded-lg transition font-medium ${
              tab === "stats" ? "bg-emerald-600 text-white" : "text-gray-600 hover:text-gray-800"
            }`}
          >
            Stats
          </button>
